...
The role of the Café has the potential to fulfill the desire of visitors to interact with each other, but at this point appears to be largely ineffective. In this space, visitors can communicate with one another by leaving messages in different chats. There are different blurbs (shown in figure #) that a user can send messages in, but there is no clear distinction for how to choose one blurb over the other. Perhaps if there were prompts to encourage conversation on a specific topic, the chat function could be more effective.

{{< q-figure id="fig-canner-04" label="Figure 4" caption="VOMA’s Cafe" alt="Mult-colored café with long gray couch, poufs, two dining tables, each with four chairs. Four chat bubbles hover over the seating areas. Glass windows open towards the gallery spaces and the exterior." >}}
